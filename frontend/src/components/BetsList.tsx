  ...
  if (!roundAddress) {
    return (
      <div className="premium-glass p-6 h-fit">
        ...
        <p className="text-white/60 text-center py-8">No active round</p>
      </div>
    );
  }
    ...
    <div className="premium-glass backdrop-blur-2xl bg-white/[0.02] p-6 h-fit">
      ...
      <div className="mb-6 bg-gradient-to-br from-purple-500/10 to-transparent rounded-2xl p-5 border border-purple-500/20">
        <div className="text-center">
          ...
          <div className="text-2xl font-bold text-white">{totalBetsCount}</div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {myBets.length > 0 ? (
            myBets.map((bet, index) => (
              <div
                key={index}
                className="bg-gradient-to-r from-purple-500/10 to-transparent rounded-xl p-4 border border-purple-500/20"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-white/40">Your bet</div>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-xs text-white/40">MON</div>
                  </div>
                </div>
              </div>
            ))
          ...
          {allTokenBets.length > 0 ? (
            allTokenBets
              ...
              .map((tokenBet, index) => (
                <div
                  key={index}
                  className="bg-white/[0.02] backdrop-blur-sm rounded-xl p-4 border border-white/5 hover:bg-white/[0.04] transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold text-white">
                        {parseFloat(formatEther(tokenBet.totalAmount)).toFixed(
                          4
                        )}
                      </div>
